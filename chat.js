...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    async function syncIdentityFromSheet() {
        ...
        try {
            ...
            const res = await fetch(`${CHAT_URL}?action=getProfile&id=${encodeURIComponent(user.username)}`);
    ...
    async function loadUsers() {
        ...
        try {
            const res = await fetch(`${CHAT_URL}?action=getUsers&kelompok=${encodeURIComponent(user.kelompok || '')}`);
            ...
            users.forEach((u) => {
                ...
                html += `
                <div class="user-card contact-item" data-username="${safeUsername}" data-nama="${safeNama}">
                    <div class="u-icon">${icons.user}</div>
                    <div class="u-info">
                        <div class="u-name">${safeNama}</div>
                        <div class="u-role">${safeRole}</div>
                    </div>
                    ${badge}
                </div>
                `;
            ...
            // Attach Listeners (ShadowRoot pakai querySelector, bukan getElementById)
            const btnGroup = shadow.querySelector('#btnGroupChat');
            if(btnGroup) {
    ...
    async function loadChat() {
        ...
        try {
            const url = `${CHAT_URL}?action=getChat&kelompok=${encodeURIComponent(user.kelompok || '')}&username=${encodeURIComponent(user.username || '')}&target=${encodeURIComponent(chatState.target || '')}`;
            ...
            messages.forEach(m => {
                ...
                <div class="msg ${isMe ? 'me' : 'other'}">
                    ${!isMe && chatState.target === 'group' ? `<span class="sender-name">${safeNama}</span>` : ''}
                    ${safePesan}
                </div>
                `;
            });
    ...
    async function sendMessage() {
        ...
        // UI Optimistic Update (escape agar aman dari XSS)
        const tempMsg = `
            <div class="msg me" style="opacity:0.7">
                ${escapeHtml(txt)} <small>⏳</small>
            </div>`;
